...
class AppUI:
    def __init__(self, root):
        ...
        self.iface_cb = ttk.Combobox(ctrl, values=self._iface_values, width=50)
        if self.iface_cb['values']: self.iface_cb.current(0)
        self.iface_cb.grid(row=0, column=1, sticky=tk.W+tk.E, padx=5)
        iface_scroll = ttk.Scrollbar(ctrl, orient=tk.HORIZONTAL, command=self.iface_cb.xview)
        self.iface_cb.config(xscrollcommand=iface_scroll.set)
        iface_scroll.grid(row=1, column=1, sticky=tk.W+tk.E, padx=5)
        ...
        ttk.Label(ctrl, text="Probes:").grid(row=2, column=0, sticky=tk.W)
        self.probe_label = ttk.Label(ctrl, text="0 loaded")
        self.probe_label.grid(row=2, column=1, sticky=tk.W, padx=5)
        ttk.Button(ctrl, text="Load...", command=self.load_probe).grid(row=2, column=2, padx=5)

        ttk.Label(ctrl, text="Log filter:").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.cb_client = ttk.Checkbutton(ctrl, text="ClientHello", variable=self.show_client)
        self.cb_client.grid(row=3, column=1, sticky=tk.W, padx=5)
        self.cb_server = ttk.Checkbutton(ctrl, text="ServerHello", variable=self.show_server)
        self.cb_server.grid(row=3, column=1, padx=(110,0), sticky=tk.W)
